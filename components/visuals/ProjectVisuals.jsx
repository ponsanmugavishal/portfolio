...
const A2 = "#ffb23f";
const CREAM = "#f3f0e8";
const LINE = "rgba(243,240,232,0.12)";
const FAINT = "rgba(243,240,232,0.45)";
    ...
    <>
      <defs>
        <pattern id={`dots-${id}`} width="16" height="16" patternUnits="userSpaceOnUse">
          <circle cx="1.5" cy="1.5" r="1" fill="rgba(243,240,232,0.08)" />
        </pattern>
...
export function WorkflowVisual() {
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" role="img" aria-label="Animated diagram of an LLM calling tools">
      ...
      {/* task node */}
      <rect x="22" y="132" width="56" height="36" rx="10" fill="#1b1b25" stroke={LINE} />
      <Label x="50" y="154" fill={CREAM}>task</Label>
      {/* llm node */}
      <circle cx="165" cy="150" r="30" fill="#1b1b25" stroke={A} strokeWidth="2" />
      ...
      <Label x="165" y="154" fill={CREAM} size={11}>LLM</Label>
      {/* tool nodes */}
      {tools.map((t, i) => (
        <g key={t.name}>
          <rect x="300" y={t.y - 17} width="86" height="34" rx="10" fill="#1b1b25" stroke={i === 2 ? A2 : LINE} />
          <Label x="343" y={t.y + 3} fill={CREAM} size={8.5}>{t.name}</Label>
        </g>
      ))}
      {/* recovery badge */}
      <g>
        <rect x="222" y="246" width="66" height="20" rx="10" fill="rgba(255,178,63,0.14)" stroke={A2} strokeWidth="1" />
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" role="img" aria-label="Animated map with SOS pin and ambulance route">
      ...
      {[[20, 20, 130, 70], [190, 20, 90, 50], [300, 130, 80, 80], [190, 140, 60, 70], [20, 110, 130, 50], [95, 210, 140, 60], [270, 230, 110, 50]].map(([x, y, w, h], i) => (
        <rect key={i} x={x} y={y} width={w} height={h} rx="8" fill="rgba(243,240,232,0.035)" stroke="rgba(243,240,232,0.06)" />
      ))}
      {/* streets */}
      <path d="M0,100 H400 M0,186 H400 M170,0 V300 M262,0 V300 M70,0 V300" stroke="rgba(243,240,232,0.07)" strokeWidth="10" />
      {/* route */}
      ...
      {/* base */}
      <rect x="52" y="236" width="36" height="36" rx="9" fill="#1b1b25" stroke={CREAM} strokeOpacity="0.4" />
      ...
      <g>
        ...
        <rect x="-11" y="-7" width="22" height="14" rx="4" fill={CREAM} />
      ...
      <g transform="translate(310,92)">
        ...
        <circle cy="-24" r="4.5" fill="#140a05" />
      </g>
      <rect x="282" y="38" width="86" height="22" rx="11" fill="#1b1b25" stroke={A} strokeOpacity="0.6" />
      <Label x="325" y="52" fill={CREAM} size={8.5}>SOS · live location</Label>
      <rect x="96" y="250" width="92" height="22" rx="11" fill="#1b1b25" stroke={LINE} />
...
export function CropVisual() {
  const bars = [
    ...
    { x: 144, label: "humidity", vals: "80;60;45;80", c: CREAM },
  ];
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" role="img" aria-label="Animated sensor readings turning into crop advice">
      ...
      {/* field rows */}
      {[0, 1, 2, 3, 4].map((i) => (
        <path key={i} d={`M${200 + i * 10},300 Q300,${210 + i * 8} ${400 - i * 6},${230 + i * 14}`} stroke="rgba(255,178,63,0.18)" strokeWidth="2" fill="none" />
      ))}
      {/* sensor bars */}
      <rect x="24" y="40" width="168" height="170" rx="16" fill="#1b1b25" stroke={LINE} />
      <Label x="108" y="62" fill={CREAM} size={9}>field data · live</Label>
      {bars.map((b) => (
        <g key={b.label}>
          <rect x={b.x} y="80" width="30" height="100" rx="6" fill="rgba(243,240,232,0.05)" />
      ...
      <g transform="translate(300,150)">
        <path d="M0,60 V0" stroke="#8fcf6a" strokeWidth="3" strokeLinecap="round" />
        <path d="M0,20 C-30,14 -34,-12 -30,-18 C-12,-16 0,0 0,20 Z" fill="#8fcf6a">
          <animateTransform attributeName="transform" type="scale" values="0.85;1;0.85" dur="3s" repeatCount="indefinite" additive="sum" />
        </path>
        <path d="M0,8 C28,4 34,-22 30,-30 C12,-28 0,-12 0,8 Z" fill="#6fb64f">
          <animateTransform attributeName="transform" type="scale" values="1;0.85;1" dur="3s" repeatCount="indefinite" additive="sum" />
        </path>
      </g>
      <rect x="252" y="228" width="98" height="24" rx="12" fill="#1b1b25" stroke={A} strokeOpacity="0.6" />
      <Label x="301" y="243" fill={CREAM} size={8.5}>✓ crop advice ready</Label>
    </svg>
  );
}
...
export function SoundVisual() {
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" role="img" aria-label="Animated sound wave switching on a light bulb">
      ...
      <rect x="178" y="112" width="72" height="76" rx="10" fill="#1b1b25" stroke={LINE} />
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <rect key={i} x={186 + i * 10} y="104" width="5" height="8" rx="1" fill="rgba(243,240,232,0.3)" />
      ))}
      <rect x="198" y="136" width="32" height="26" rx="4" fill="rgba(243,240,232,0.08)" />
      ...
      <rect x="276" y="134" width="32" height="32" rx="6" fill="#1b1b25" stroke={LINE} />
      <path d="M284,156 L300,146" stroke={CREAM} strokeWidth="2" strokeLinecap="round">
      ...
      <g transform="translate(352,140)">
        <circle r="34" fill={A2} opacity="0">
          <animate attributeName="opacity" values="0;0.35;0.35;0" keyTimes="0;0.1;0.6;0.7" dur="3s" repeatCount="indefinite" />
        </circle>
        <path d="M-14,4 C-22,-14 -10,-30 0,-30 C10,-30 22,-14 14,4 C10,10 8,14 8,18 H-8 C-8,14 -10,10 -14,4 Z" fill="#26262f" stroke={CREAM} strokeOpacity="0.5">
          <animate attributeName="fill" values="#26262f;#ffd27a;#ffd27a;#26262f" keyTimes="0;0.1;0.6;0.7" dur="3s" repeatCount="indefinite" />
        </path>
        <rect x="-8" y="20" width="16" height="10" rx="2" fill="rgba(243,240,232,0.4)" />
      </g>
